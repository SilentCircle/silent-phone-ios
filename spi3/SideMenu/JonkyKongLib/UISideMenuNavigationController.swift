...
open class UISideMenuNavigationController: UINavigationController {
    ...
    override open func pushViewController(_ viewController: UIViewController, animated: Bool) {
        ...
        //ET: hack in support for container vc
        guard let navigationController = presentingViewController?.childViewControllers[0] as? UINavigationController else {
            print("Check assumptions about navigationController being 0th childVC of \(String(describing: presentingViewController))")
            
            // ET: nest this original check for tabBarCon within setting the navCon in containing guard statement
            let tabBarController = presentingViewController as? UITabBarController
            guard ((tabBarController?.selectedViewController ?? presentingViewController) as? UINavigationController) != nil else {
