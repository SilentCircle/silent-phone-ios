...
void signalHandler(int signal) {
    ...
    NSDictionary *userInfo = @{
                               UncaughtExceptionHandlerAddressesKey: [RavenClient backtrace]
                               };
    ...
    NSString *excReason = [NSString stringWithFormat:NSLocalizedString(@"Signal %d (%@) was raised.", nil), signal, signalName];
...
- (NSDateFormatter *)dateFormatter {
    if (!_dateFormatter) {
        ...
        _dateFormatter.calendar = [ChatUtilities utilitiesInstance].gregorianCalendar;
    }
...
- (void)captureMessage:(NSString *)message
    ...
    NSArray *stacktrace = [NSArray array];
    NSString *culprit = @"";
    if (method && file && line) {
...
- (void)captureMessage:(NSString *)message
    ...
    if ([[Reachability reachabilityForInternetConnection] currentReachabilityStatus] == NotReachable || !sendNow) {
...
- (void)captureException:(NSException *)exception additionalExtra:(NSDictionary *)additionalExtra additionalTags:(NSDictionary *)additionalTags sendNow:(BOOL)sendNow {
    ...
    if(exception.userInfo && [exception.userInfo objectForKey:UncaughtExceptionHandlerAddressesKey])
        callStack = [exception.userInfo objectForKey:UncaughtExceptionHandlerAddressesKey];
    else if([exception callStackSymbols])
...
- (void)processSavedReports {
    ...
    if (reports != nil && [reports count]) {
        ...
        for (NSDictionary *data in reports) {
            ...
            // Don't send more than 5 reports
            if(sentReports > 4)
                break;
...
- (void)setupExceptionHandler {
    ...
    [self processSavedReports];
    
    // Setup uncaught exception handler
    NSSetUncaughtExceptionHandler(&exceptionHandler);
    
    // Setup signal handlers
    // ref: http://www.cocoawithlove.com/2010/05/handling-unhandled-exceptions-and.html
